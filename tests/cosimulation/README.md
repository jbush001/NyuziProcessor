...
generated by a utility. The cosimulator only works in single-core configurations.
...
Verilator is a 2-state simulator. Whereas a single bit in a standard verilog simulator can have 4 states: 0, 1, X, and Z,
Verilator only supports 0 and 1. Actually, this isn't so bad, since the way Verilog handes X and Z are fundamentally 
flawed by design. This paper http://www.arm.com/files/pdf/Verilog_X_Bugs.pdf gives a good description why.

As an alternative, Verilator allows assigning 1 or 0 randomly to bits that would normally be assigned X.  This
is a nice feature, since it allows catching classes of failures that would be impossible in a normal
Verilog simulator. However, it also means that programs will potentially run differently run-to-run if all 
signals are not reset (SRAMs, for example, are not).  In order to get a consistent run, you can modify 
rtl/vn/testbench/verilator_main.cpp to hardcode the random seed to a fixed value.  This is useful when performing
multiple runs to debug a specific failure.
