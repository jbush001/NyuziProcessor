...
def generate_binary_arith(outfile):
    ...
    Args:
        outfile: File
           File that the instruction should be appended to.
...
def generate_device_io(outfile):
    ''' Write a random single device memory load or store instruction.

    This accesses the range (0xffff0000-0xffffffff).
...
def generate_branch(outfile):
    '''Write a single branch instruction to outfile.

    This will use a relative forward branch to an anonymous label 1-6
    instructions away.
...
def generate_computed_pointer(outfile):
    ...
    'computed pointer' registers.

    These are guaranteed to be valid memory locations
