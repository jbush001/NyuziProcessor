...
int Encode(unsigned char *output, const unsigned char *input, int inputLength)
{
	...
	for (len = 0; len < F && inputOffset < inputLength; len++)
		text_buf[r + len] = input[inputOffset++];  /* Read F bytes into the last F bytes of
			the buffer */
	...
	do {
		...
		if ((mask <<= 1) == 0) {  /* Shift mask left one bit. */
			for (i = 0; i < code_buf_ptr; i++)  /* Send at most 8 units of */
				output[outputOffset++] = code_buf[i];     /* code together */
			codesize += code_buf_ptr;
			code_buf[0] = 0;  code_buf_ptr = mask = 1;
		}
		last_match_length = match_length;
		for (i = 0; i < last_match_length && inputOffset < inputLength; i++) {
...
int Decode(unsigned char *output, const unsigned char *input, int inputLength)
{
	int  i, j, k, r, c;
	unsigned int  flags;
	int inputOffset = 0;
...
char kRawText[] = "Four score and seven years ago our fathers brought forth on this continent, a new nation, conceived in Liberty, and dedicated to the proposition that all men are created equal."
