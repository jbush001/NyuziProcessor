...
memcpy:             	move s6, s0                # Save source pointer, which we will return
                    	...
                    	and s4, s1, 63
                    	cmpeq_i s5, s3, s4
                    	bz s5, copy_word_check    # Not aligned, see if we can copy words
...
copy_vector_lead_in: 	and s4, s0, 63          # Aligned yet?
                    	bz s4, copy_vector  # Yes, time to do big copies
                    	bz s2, copy_done    # Bail if we are done.
                    	load_u8 s4, (s1)
                    	store_8 s4, (s0)
                    	add_i s0, s0, 1
                    	add_i s1, s1, 1
                    	sub_i s2, s2, 1
                    	b copy_vector_lead_in

                    	# Copy entire vectors at a time
copy_vector:        	cmplt_u s4, s2, 64      # 64 or more bytes left?
                    	bnz s4, copy_words    # No, attempt to copy words
                    	load_v v0, (s1)
                    	store_v v0, (s0)
                    	add_i s0, s0, 64
                    	add_i s1, s1, 64
...
copy_word_check:    	and s3, s0, 3
                    	and s4, s1, 3
                    	cmpeq_i s5, s3, s4
                    	bz s5, copy_remain_bytes    # Not aligned, need to do it the slow way

copy_word_lead_in:    	and s4, s0, 3           # Aligned yet?
                    	bz s4, copy_words   # If yes, start copying
                    	bz s2, copy_done    # Bail if we are done
                    	load_u8 s4, (s1)
                    	store_8 s4, (s0)
                    	add_i s0, s0, 1
...
copy_words:            	cmplt_u s4, s2, 4           # 4 or more bytes left?
                    	bnz s4, copy_remain_bytes # If not, copy tail
                    	load_32 s4, (s1)
                    	store_32 s4, (s0)
                    	add_i s0, s0, 4
                    	add_i s1, s1, 4
...
copy_remain_bytes:    	bz s2, copy_done
                    	load_u8 s4, (s1)
                    	store_8 s4, (s0)
                    	add_i s0, s0, 1
                    	add_i s1, s1, 1
